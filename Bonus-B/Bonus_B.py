...
for cn in classifier_name:
    plt.errorbar(range(1, 101), acc_results[cn], yerr=acc_errors[cn], marker='o', linestyle='-', label=f'{cn} ACC', capsize=5)
    plt.errorbar(range(1, 101), mcc_results[cn], yerr=mcc_errors[cn], marker='s', linestyle='--', label=f'{cn} MCC', capsize=5)    
plt.xlabel('Features num')
plt.ylabel('norm_num')
plt.title('Incremental Feature Selection of SVM/Nbayes/KNN')
plt.legend()
plt.grid(True)
plt.show()
