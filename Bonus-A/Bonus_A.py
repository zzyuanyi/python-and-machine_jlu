...
result_dict={}
ten_results = {clf: [] for clf in ['SVM', 'Nbayes', 'KNN']}
for feature_label,feature_data in feature_sets.items():
    ...
    for _ in range(10):     
        ...
        for md,norm in result.items():
            ten_results[md].append(norm)
    avg_results = {md: np.mean(ten_results[md], axis=0) for md in ten_results}
    std_errors = {md: np.std(ten_results[md], axis=0) for md in ten_results}
